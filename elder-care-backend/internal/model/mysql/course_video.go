...
func CreateCourseVideo(ctx context.Context, tx *gorm.DB, catalogId string, videoUrl string, format, language, size, duration string, uploadAt time.Time) (*CourseVideo, *terror.Terror) {
	...
	if retGorm.Error != nil {
		errMsg := tlog.E(ctx).Err(retGorm.Error).Msgf("Create course video (catalog id: %s, video url: %s, format: %s, language: %s, size: %s, duration: %s, upload at: %v) err (db create %v)",
			catalogId, videoUrl, format, language, size, duration, uploadAt, retGorm.Error)
...
func FindCourseVideosByCatalog(ctx context.Context, catalogIds []string) ([]*CourseVideo, *terror.Terror) {
	courseVideosDB := make([]*CourseVideo, 0)

	retGorm := serverClient.DB(ctx, runMode).Where("catalog_id IN (?)", catalogIds)

	retGorm = retGorm.Order("created_at DESC").Find(&courseVideosDB)
	if retGorm.Error != nil {
		errMsg := tlog.E(ctx).Err(retGorm.Error).Msgf("Find course videos by catalog (catalog ids: %v) err (db find %v)",
...
func UpdateCourseVideo(ctx context.Context, tx *gorm.DB, videoId string, videoUrl string, format, language, size, duration string, uploadAt time.Time) *terror.Terror {
	...
	if retGorm.Error != nil {
		errMsg := tlog.E(ctx).Err(retGorm.Error).Msgf("Update course video (video id: %s, video url: %s, format: %s, language: %s, size: %s, duration: %s, upload at: %v) err (db updates %v)",
			videoId, videoUrl, format, language, size, duration, uploadAt, retGorm.Error)
		errx := terror.NewTerror(ctx, retGorm.Error, constant.ErrorCodeMysqlServerAbnormal, errMsg)
