...
// ClientAuthMiddleware returns a placeholder authentication middleware.
func ClientAuthMiddleware() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if accessToken == "" {
			errMsg := tlog.E(ctx).Msg("Client auth middleware err (access token missing)")
		...
		if errx != nil {
			errMsg := tlog.E(ctx).Err(errx).Msgf("Client auth middleware err (check jwt access token %v)",
				errx)
		...
		if !isAdmin {
			errMsg := tlog.E(ctx).Msgf("Client auth middleware (user id: %s) err (permission forbidden)",
				userId)
		...
		if errx != nil {
			errMsg := tlog.E(ctx).Err(errx).Msgf("Client auth middleware (method: %s, path: %s) err (db find api resource %v)",
		...
		if apiResourceDB == nil || apiResourceDB.AuthRequired != dbmodel.IsValueTrue {
			errMsg := tlog.E(ctx).Msgf("Client auth middleware (method: %s, path: %s) err (api resource unauthorized)",
		...
		if errx != nil {
			errMsg := tlog.E(ctx).Err(errx).Msgf("Client auth middleware (role id: %s, method: %s, path: %s) err (db find authorized api resource %v)",
		...
		if apiResource2DB == nil {
			errMsg := tlog.E(ctx).Msgf("Client auth middleware (role id: %s, method: %s, path: %s) err (permission forbidden)",
